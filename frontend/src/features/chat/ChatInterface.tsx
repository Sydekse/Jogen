      ...
      <div className="flex-1 overflow-y-auto p-3 sm:p-4 space-y-3 sm:space-y-4">
        {activeSession.messages.map((msg) => (
          <div key={msg.id} className={`flex ${msg.sender === "user" ? "justify-end" : "justify-start gap-2.5 items-start"}`}>
            ...
            <div className={cn(
              "px-3.5 py-2.5 sm:px-4 sm:py-3 text-xs sm:text-sm max-w-[90%] sm:max-w-[85%] leading-relaxed",
              msg.sender === "user" 
        ...
        {isLoading && (
          <div className="flex gap-2.5 items-start justify-start">
            ...
            <div className="bg-muted rounded-2xl rounded-tl-sm px-4 py-3 text-xs sm:text-sm max-w-[85%] text-muted-foreground flex items-center gap-2">
      ...
      <div className="p-3 sm:p-4 border-t border-border bg-card shrink-0 sticky bottom-0 z-20">
        <form onSubmit={handleSendMessage} className="flex gap-2">
          ...
          <button 
            ...
            className="w-10 h-10 sm:w-11 sm:h-11 rounded-xl bg-primary flex items-center justify-center shrink-0 disabled:opacity-50 transition-opacity"
          >
